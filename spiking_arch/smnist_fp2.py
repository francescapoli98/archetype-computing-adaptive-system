...
for i in range(args.trials):
    ...
    for images, labels in tqdm(train_loader):
        ...
        # act_p.append(output)
        # vel_p.append(velocity)
        # mem_p.append(u)
        # spk_p.append(spk)
    # print('activations shape: ', activations.shape, '\nvel_p shape: ', vel_p.shape, '\nmem_p shape: ', mem_p.shape, '\nspk_p shape: ', spk_p.shape)
    # plot_dynamics(activations, vel_p, mem_p, spk_p, args.resultroot) 
    # print('datatypes and shapes: \n output: ', type(output), output.shape, '\n velocity: ', type(velocity),velocity.shape, '\n membrane potential: ', type(u), u.shape, '\n spikes: ', type(spk), spk.shape)
    plot_dynamics(output, velocity, u, spk, args.resultroot) 

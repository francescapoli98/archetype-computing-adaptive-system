...
class SpikingNeuronsAndOscillatorsNet(nn.Module):
    """
    ...
    def __init__(
        ...
        rho: float,
        input_scaling: float,
        proportion: float, #### PARAMETER TO PROPORTIONATE THE NUMBER OF RANDOMLY GENERATED OSCILLATORS AND SPIKING NEURONS
        topology: Literal[
    ...
    def cell(
        ...
        hz = hz + self.dt * (
            ...
            + (1-self.prop)*(- self.gamma * hy - self.epsilon * hz) #### MECHANICAL OSCILLATORS layer 
            + self.prop*(lif_unit()) #SPIKING NEURONS layer
        )
    ...
    def lif_unit(self, mem, cur=0, threshold=1.0, time_step=1e-3, R=5.1, C=5e-3):
        tau_mem = R*C
